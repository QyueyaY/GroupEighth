...
public class ContentServiceImpl implements IContentService {
    ...
    @Override
    public PageInfo<ContentVo> getArticles(String keyword, Integer page, Integer limit, String order) {
        PageHelper.startPage(page, limit);
        ContentVoExample contentVoExample = new ContentVoExample();
        ContentVoExample.Criteria criteria = contentVoExample.createCriteria();
        criteria.andTypeEqualTo(Types.ARTICLE.getType());
        criteria.andStatusEqualTo(Types.PUBLISH.getType());
        criteria.andTitleLike("%" + keyword + "%");
        if (Boolean.valueOf(order)) {//order为true时以点击量排序
            contentVoExample.setOrderByClause("hits desc");
        } else {//order为false时以创建时间排序
            contentVoExample.setOrderByClause("created desc");
        }
        List<ContentVo> contentVos = contentDao.selectByExampleWithBLOBs(contentVoExample);
        return new PageInfo<>(contentVos);
    ...
    @Override
    public PageInfo<ContentVo> getArticlesWithpage(ContentVoExample commentVoExample, Integer page, Integer limit, String order) {
        PageHelper.startPage(page, limit);
        if (Boolean.valueOf(order)) {
            commentVoExample.setOrderByClause("hits desc");
        } else {
    ...
    @Override
    public PageInfo<ContentVo> getArticlesWithCate(ContentVoExample contentVoExample, int page, int limit, String categorites) {
        PageHelper.startPage(page, limit);
        if (categorites.equals("简单")) {
            contentVoExample.setOrderByClause("简单");
        } else if(categorites.equals("普通")){
            contentVoExample.setOrderByClause("普通");
        } else if(categorites.equals("困难")){
            contentVoExample.setOrderByClause("困难");
        }
        List<ContentVo> contentVos = contentDao.selectByCateWithBLOBs(contentVoExample);
        return new PageInfo<>(contentVos);
    }
